...
namespace CloudPart1.Services
{
    public class TableStorageService
    {
        ...
        public TableStorageService(string connectionString)
        {
            ...
            _tableClient=new TableClient(connectionString, "Item");
            ...
            _CheckoutTableClient = new TableClient(connectionString, "Checkout");
        }
        ...
        public async Task<CustomerProfile?> GetCustomerProfileAsync(string partitionKey, string rowKey)
        {
            try
            {
                var response = await _CustomerProfileTableClient.GetEntityAsync<CustomerProfile>(partitionKey, rowKey);
                return response.Value;
            }
        ...
        public async Task<List<CustomerProfile>> GetAllCustomerProfileAsync()
        {
            var profiles = new List<CustomerProfile>();

            await foreach (var profile in _CustomerProfileTableClient.QueryAsync<CustomerProfile>())
            {
        ...
        public async Task<CustomerProfile?> GetCustomerProfileByEmailAsync(string email)
        {
            try
            {
                return await _CustomerProfileTableClient.GetEntityAsync<CustomerProfile>("CustomerProfiles", email);
            }
            catch (RequestFailedException ex) when (ex.Status == 404)
            {
                return null; 
